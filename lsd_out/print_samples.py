            ...
            for rep in reps:
                for k, channel in enumerate(mesonic_channels):
                    ...
                    if kernel == 'HALFNORMGAUSS':
                        ...
                        kernel2 = kernel
                    spdens_outdir = f'./{ensemble}_{rep}_{channel}_s0p{decimal_as_int}_{kernel}_Nsource{Nsource}_Nsink{Nsink}'
                    ...
                    if rep == 'fund':
                        outdir = f'../input_fit/{ensemble}/{channel}_{rep}/{kernel2}/{channel}_{rep}'
                        part_outdir = f'../input_fit/{ensemble}/{channel}_{rep}/{kernel2}/fit_results.txt'
                    if rep == 'anti':
                        outdir = f'../input_fit/{ensemble}/{channel}_as/{kernel2}/{channel}_as'
                        part_outdir = f'../input_fit/{ensemble}/{channel}_as/{kernel2}/fit_results.txt'
