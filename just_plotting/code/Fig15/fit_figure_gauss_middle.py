...
gauss_data = np.loadtxt("gauss_curves_middle.txt")
...
data_points = np.loadtxt("sp_dens_datapoints_gauss_middle.txt")

# Extracting energy and spdens values
energy, spdens, err_spdens = data_points[:,0], data_points[:,1], data_points[:,2]

# Plot data points
plt.errorbar(energy / mpi, spdens, yerr=err_spdens, fmt='o', color='black', markersize=3.0, elinewidth=1.2, label='$\sigma = 0.30m_{\mathrm{PS}}$\n$N_{\mathrm{source}} = 40$\n$N_{\mathrm{sink}} = 40$')

# Set labels and legend
plt.xlabel('$\omega / m_{\mathrm{PS}}$', fontsize=14)
plt.ylabel('$\hat{\\rho}_{\sigma}(\omega)$', fontsize=14)
plt.title('$N_t = 64$', fontsize=13)
...
plt.savefig(
            '../../../plots/fit_results_NEWPAUL_nt64_fund_g5_double_2.png',
